...
async function showData(){
    const productList = (await fetch('http://localhost:9092/almacen').then(response => response.text())).split('\n');
    let html="";
...
async function AddData(){
    if(validateForm() == true){
        ...
        await fetch('http://localhost:9092/almacen', {
            method: 'POST',
            headers: {
...
async function deleteProduct(index){
    await fetch('http://localhost:9092/almacen/' + index, {
        method: 'DELETE'
    });
    showData();
}
...
async function updateProduct(index){
    ...
    let product = await fetch('http://localhost:9092/almacen/' + index).then(response => response.text());
    ...
    document.querySelector("#Update").onclick = async function(){
        if(validateForm() == true){
            await fetch('http://localhost:9092/almacen/' + index + "/true", {
                method: 'PUT',
                headers: {
                    'Content-Type': 'text/plain'
                },
